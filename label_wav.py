...
def run_graph(wav_data, labels, input_layer_name, output_layer_name,
              num_top_predictions, pass_tensor = False):
  ...
  with tf.compat.v1.Session() as sess:
    ...
    if pass_tensor == True:
        wav_data_eval = wav_data.eval()  
        wav_data = wav_data_eval
        
      
    print(sess.graph.get_tensor_by_name(input_layer_name))
    ...
    for node_id in top_k:
      ...
      scorearray.append([node_id, human_string, score])
...
def label_wav(wav, labels, graph, input_name, output_name, how_many_labels, pass_tensor = False):
  ...
  results = run_graph(wav_data, labels_list, input_name, output_name, how_many_labels, pass_tensor)

  return results

def force_load(wav, labels, graph, input_name, output_name, how_many_labels, pass_tensor = False):
